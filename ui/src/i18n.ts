import i18n from "i18next";
import { initReactI18next } from "react-i18next";

// Bilingual UI (German default, English fallback). Toggle in the header.
const en = {
  nav: { grid: "Grid", loads: "Loads", live: "Live" },
  app: { defaultGrid: "{{name}} (default)", noGrid: "no grid",
         busShort: "bus", loadsShort: "loads" },

  grid: {
    title: "Generate a grid",
    subtitle: "Synthetic German distribution grids (ding0) on real geography — pick a type, size and area character.",
    voltageLevel: "Voltage level", mv: "Medium (20 kV)", lv: "Low (0.4 kV)",
    areaCharacter: "Area character",
    rural: "Rural", suburban: "Suburban", urban: "Urban", user: "Custom",
    ieee: "IEEE", cigre: "CIGRE", kerber: "Kerber",
    ruralHint: "low density, long feeders", suburbanHint: "medium density", urbanHint: "high density, compact",
    userHint: "own grids drawn with gridedit",
    approxNodes: "Approx. nodes",
    noMatch: "No {{voltage}} {{character}} grid in the library yet — try another combination.",
    adjust: "Adjust the controls to pick a grid.",
    building: "Building grid…",
    transformer: "Transformer: {{kva}} kVA",
    notes: "{{count}} import note(s)",
    configure: "Configure loads →",
    failed: "Failed to load grids:", loadingLib: "Loading grid library…",
    noLibrary: "No grid library found on the server.",
    runScript: "Run scripts/build_grid_library.py to generate one from the OEP.",
    nodes: "{{count}} nodes",
    kBuses: "buses", kLines: "lines", kTrafos: "trafos", kLoads: "loads",
    ruralAdj: "Rural", suburbanAdj: "Suburban", urbanAdj: "Urban",
    nameMv: "{{adj}} MV grid · district {{district}}",
    nameLv: "{{adj}} LV grid · {{lvid}}",
  },

  loads: {
    title: "Generate loads",
    subtitle: "Grid {{grid}}. Profiles are real LPG households assigned across the loads.",
    noLpg: "No LPG library on the server — you can still run with synthetic placeholder loads.",
    archetypes: "Household archetypes ({{count}} selected)",
    kwhYr: "{{kwh}} kWh/yr",
    mode: "Assignment mode", roundRobin: "Round-robin (even spread)", random: "Random (seeded)",
    seed: "Random seed", scale: "Scale factor ({{scale}}×)", pf: "Power factor ({{pf}})",
    evPen: "🔌 EV penetration ({{pct}}%)",
    evHint: "Share of homes charging an EV in the evening (uncontrolled).",
    wallbox: "Wallbox power", ev37: "3.7 kW (slow)", ev11: "11 kW (wallbox)", ev22: "22 kW (fast)",
    evMix: "random: 3.7 / 11 / 22 kW",
    pvSizing: "PV sizing",
    pvEqual: "equal size (kWp slider)",
    pvMix: "random size and orientation",
    pvPen: "☀️ PV penetration ({{pct}}%)", pvSize: "PV size per home ({{kwp}} kWp)",
    preview: "Preview", applyRun: "Apply & run →", placeholder: "Placeholder",
    placeholderHint: "Skip LPG, use placeholder loads",
    aggregate: "Aggregate daily load",
    previewHint: "Click “Preview” to simulate the feeder’s net load curve.",
    kLoads: "loads", kEvs: "EVs", kPv: "PV systems", kNetPeak: "peak power", kMinNet: "min net",
    netDesc: "Net feeder demand (solid) over 24 h, kVA",
    netDescPv: " — dashed = gross load before PV",
    reverseFlow: " Net goes negative → midday reverse flow (PV export).",
    archUsed: "Archetypes used: {{list}}.",
  },

  live: {
    map: "Map", schematic: "Schematic", values: "Values", streets: "Streets", reset: "Reset view",
    mapTitle: "Real OpenStreetMap basemap at the grid's coordinates",
    valuesTitle: "Show live current on lines and voltage/power at nodes (SCADA-style readings)",
    day: "day {{day}} · {{time}}", notConverged: " · not converged",
    gridInfo: "{{name}} · {{buses}} buses · ws {{ws}}",
    vminmax: "V min / max", maxLine: "max line loading", maxTrafo: "max trafo loading",
    totalLoad: "total load", generation: "generation", slack: "slack (import)",
    losses: "losses", solveTime: "solve time", na: "n/a",
    selectHint: "Click an element for actions (graph, battery, meter). Ctrl/⌘+click pins its daily graph directly.",
    legLow: "<50%", legMed: "<80%", legHigh: "<100%", legOver: "overload",
    underV: "under-V", okV: "ok", overV: "over-V",
    mapHint: "Lines colored green→red by loading; nodes reddened by voltage Δ. Scroll to zoom, drag to pan.",
    schematicHint: "Line width ∝ current. Scroll to zoom, drag to pan.",
    play: "▶ Play", pause: "⏸ Pause",
    time: "Time: {{time}}", dayLabel: "Day", dayTitle: "Real PV day (measured)",
    stepDur: "Step duration", stepDurTitle: "Real seconds per simulation step",
    failedNet: "Failed to load network:", loadingNet: "Loading network…",
    reveal: "Power flow on", revealOff: "Power flow off",
    revealTitle: "Show or hide the true power flow (off = only placed meters reveal values)",
    observedNote: "Measured values only — from placed meters.",
    truthHidden: "Ground truth hidden by the server (strict observability).",
    measuredVmm: "measured V min / max", measuredTrafo: "measured max trafo",
    measuredLoad: "measured load (Σ)", coverage: "meter coverage", groundTruth: "ground truth",
    estimate: "Estimate", estimateTitle: "WLS state estimation from the placed meters + grid model",
    estCaption: "state estimate", estErrV: "est. error max |ΔV|",
    estSolve: "estimation time",
  },

  graph: { uhr: "{{time}}", est: "Estimate", meas: "Measured" },
  rec: {
    heading: "Data export",
    exportDays: "Export days",
    exportDaysDots: "Export days…",
    exportTitle: "Export days",
    exportStart: "Start export",
    days: "Number of days",
    estimate: "Include the state estimate",
    exportHint: "Simulates the days offline with the current setup — much faster than the live clock. With the estimate enabled in TAF 9/10/14 mode it takes considerably longer.",
    estimateTitle: "Include the state estimate (slower on TAF 9/10/14 setups)",
    exportRunning: "Bulk export running",
    expChip: "Export {{pct}} %",
    cancel: "Cancel",
    record: "Record live session",
    recordStop: "Stop recording",
    recChip: "REC {{n}}",
    recordings: "Recordings",
    none: "no recordings yet",
    delete: "Delete recording",
    steps: "steps",
    error: "Error",
  },
  view: {
    noMeter: "no meter at this element — place one to see measured values",
    noLineMeter: "lines carry no meters — currents only appear in the power-flow or estimate view",
  },
  ctrl: {
    title: "Overload controller",
    limit: "limit",
    limitTitle: "Curtail while the loading of the domain exceeds this",
    evF: "EV charging at",
    pvF: "PV feed-in at",
    active: "curtailing (loading above limit)",
    idle: "standing by — no overload",
    seen: "sees loading",
    srcMeter: "measured",
    srcEst: "estimated",
    blind: "blind — no measurement data (place meters)",
  },
  axis: {
    time: "Time / h",
    power: "Power / kW",
    apparent: "Power / kVA",
    voltage: "Voltage / V",
    current: "Current / A",
    soc: "State of charge / %",
    price: "Price / €/MWh",
  },

  mbar: {
    netz: "Grid",
    datei: "File",
    segTruth: "Power flow",
    segObserved: "Measured",
    segEst: "Estimate",
    estConfigDots: "Estimation policy…",
    sim: "Simulation",
    view: "View",
    meas: "Metering",
    est: "Estimation",
    help: "Help",
    netzStudio: "Grid & loads…",
    toLive: "Live view",
    tabsHdr: "Workspace",
    tabLive: "Live",
    tabStudio: "Grid & loads",
    activeHdr: "Active",
    start: "Start",
    pause: "Pause",
    saved: "Scenario saved",
    viewOnlyLive: "Applies to the live view.",
    displayHdr: "Display",
    sightHdr: "Perspective",
    sightTruth: "Power flow (ground truth)",
    sightObserved: "Measured",
    taf9Sub: "1-min V/P/Q/I",
    taf7Sub: "15-min mean P",
    manual: "User manual (PDF)",
    source: "Source on GitHub",
  },
  netz: {
    step1: "1 · Choose grid",
    step2: "2 · Loads & generation",
    step3: "3 · Check & start",
    import: "Import grid…",
    imported: "Imported: {{name}}",
    importErr: "Import failed:",
    refnets: "Reference feeders (IEEE · CIGRE · Kerber)",
    library: "Library",
    own: "Your grids (gridedit)",
    noOwn: "none yet — export from gridedit or import a file",
    pickHint: "Pick or import a grid on the left.",
    kTrafo: "Transformer",
    kHouseholds: "Households",
    kTrafoPeak: "max. trafo loading",
    mfh: "Multi-family buildings (3–6 households each)",
    mfhHint: "Applied in suburban and urban grids — several load profiles are summed per building.",
    mfhUsed: "{{mfh}} multi-family buildings · {{hh}} households in total",
    trafoLine: "Dashed line: transformer rating {{kva}} kVA.",
    noTrafoRating: "No transformer rating known for this grid.",
    advanced: "Advanced (archetypes, seed, scaling)",
  },
  est: {
    tab: "Estimation",
    tabTitle: "Configure what the state estimation may use",
    heading: "State-estimation policy",
    intro: "What does the operator's WLS estimation trust? Defaults follow real DSO practice.",
    pseudoGroup: "Pseudo-measurements",
    pvPseudo: "Use PV pseudo-measurements",
    pvPseudoHint: "Usually OFF: plants differ in orientation, a fleet-wide day shape is not trusted. Plant sizes still widen the uncertainty.",
    evPseudo: "Use EV-charging pseudo-measurements",
    evPseudoHint: "Usually OFF: charging is stochastic; no standard profile exists.",
    zeroInj: "Use zero injection (junctions/cabinets)",
    zeroInjHint: "Structural knowledge: nothing is consumed at joints and cabinets. Without it the system may become unobservable.",
    basisGroup: "Load assumption (households)",
    basisProfile: "True daily means (idealized)",
    basisSlp: "Standard load profile (uniform)",
    basisProfileHint: "Each customer's real daily mean — better knowledge than a DSO actually has.",
    basisSlpHint: "Every household assumed identical (annual kWh below) — realistic DSO knowledge.",
    slpKwh: "Assumed annual consumption (kWh)",
    stdPct: "Pseudo uncertainty: ±{{pct}} % of daily peak",
    stdHint: "How strongly the estimation trusts the load assumptions vs. the meters.",
    hierGroup: "Vertical estimation (MV/LV)",
    hierAuto: "Automatic (hierarchical on districts)",
    hierOn: "Hierarchical (cells → MV level)",
    hierOff: "Monolithic (one WLS over everything)",
    hierHint: "Hierarchical = every secondary-substation cell estimates locally; its boundary flow feeds the MV-level estimate — the way a DSO actually layers it. Only affects grids with cells.",
  },

  ront: {
    title: "rONT",
    tap: "tap position",
    seenV: "sees busbar voltage",
    band: "holds the busbar between {{lo}} and {{hi}} V",
    target: "setpoint",
    targetTitle: "Busbar voltage setpoint — the OLTC steps until the observed voltage is inside the deadband",
  },

  cells: {
    heading: "Cells",
    back: "district view",
    hint: "Click a cell: the map zooms to it and its station is pinned. ● green = quiet, amber = dimming on signal, red = station overloaded, grey = no reading.",
  },

  ampel: {
    heading: "Grid traffic light",
    coordinator: "MV coordinator",
    signalEv: "EV signal (all cells)",
    signalPv: "PV signal (all cells)",
    noCoord: "No MV coordinator placed — click the substation (UW) bus to add one.",
    cells: "ONS cells",
    boxes: "Steuerboxen (cell controllers)",
    dimming: "currently dimming",
  },
  node: {
    title: "Node {{name}}", power: "Power", voltage: "Voltage",
    none: "No load or generation at this node.",
    residential: "Residential", ev: "EV", pv: "PV", voltageSeries: "Voltage",
    wind: "Wind power", biogas: "Biogas", gen: "Generation",
  },
  line: { title: "Line {{name}}", current: "Current", rated: "rated {{a}} A",
          none: "No current (line out of service?)." },
  trafo: { title: "Transformer {{name}}", power: "apparent power S",
           rated: "rated {{kva}} kVA", ratedNeg: "−{{kva}} kVA",
           none: "No power data (transformer out of service?)." },

  bat: {
    heading: "Batteries", none: "No batteries yet.",
    self: "Self-sufficiency", peak: "Peak-shaving", price: "Price",
    busN: "bus {{bus}}", addHint: "Select a single node or the transformer to add a battery there.",
    apply: "OK", sizeTitle: "Battery size (energy / power)",
    add: "＋ Battery @ bus {{bus}}", rowTitle: "Show daily SOC / power",
    title: "Battery bus {{bus}} · {{mode}}",
    soc: "State of charge", pwr: "Charge (+) / discharge (−)", dayahead: "Day-ahead price",
    sell: "{{v}} (sell)", buy: "{{v}} (buy)",
    noData: "No power data (battery out of service?).",
  },

  meas: {
    heading: "Measurements", none: "No measurements placed — the grid is unobservable.",
    smartMeter: "Smart meter", trafoMeter: "Transformer meter",
    node: "node {{bus}}", trafo: "transformer {{trafo}}",
    coverage: "Coverage", coverageVal: "{{nodes}}/{{totalNodes}} nodes · {{trafos}}/{{totalTrafos}} trafos",
    addNodeHint: "Select a node or transformer to place a measurement there.",
    addNode: "📟 Meter @ node {{bus}}", addTrafo: "📟 Meter @ transformer {{trafo}}",
    remove: "remove measurement",
    modeFull: "TAF9/10/14", modeStd: "TAF7",
    modeFullTitle: "TAF 9/10/14 — grid-state telemetry: voltage, P, Q and current every minute",
    modeStdTitle: "TAF 7 — metered load profile: only the 15-minute mean active power",
    deviceModeTitle: "This device's metering: TAF 9/10/14 = 1-min V/P/Q/I, TAF 7 = 15-min mean P only",
    perDeviceHint: "Applies to all placed devices and sets the default for new ones — switch individual devices in their section.",
    presets: "Bulk", presetAllNodes: "All nodes", presetSubstations: "Substations",
    presetAllTrafos: "All trafos", clear: "Clear all",
    presetDigitalStations: "Digital secondary substations",
    presetDigitalStationsSub: "one station meter per cell ({{n}} cells)",
    strict: "Strict mode — the server hides ground truth.",
    unknown: "unknown",
  },

  menu: {
    graph: "Daily profile", addMeter: "Place meter", removeMeter: "Remove meter",
    addBattery: "Add battery", addBatteryTrafo: "Battery @ LV busbar",
    addController: "Place controller", addControllerTrafo: "Grid controller @ station",
    addControllerCell: "Steuerbox (cell controller)",
    addControllerMv: "Netzampel coordinator (MV)",
    removeController: "Remove controller",
    addRont: "Activate rONT (OLTC)",
    removeRont: "Remove rONT",
    removeBattery: "Remove battery",
    addPv: "Add PV system", addEv: "Add EV charging",
    removePv: "Remove PV system", removeEv: "Remove EV charging",
    addExt: "Attach external source", removeExt: "Remove external source",
  },

  ext: {
    title: "External source",
    applied: "applied value",
    age: "last telegram",
    ageVal: "{{s}} s ago",
    never: "no telegram yet",
    stale: "source silent for {{s}} s — {{policy}}",
    staleNever: "no telegram received yet — {{policy}}",
    policyHold: "the last value is held",
    policyZero: "the value drops to 0",
    hold: "hold on timeout",
    zero: "zero on timeout",
    bound: "value bound",
    history: "received",
    feedHint: "Feed live values: PUT /api/ext/{{id}}/value with {\"p_kw\": …} (signed: + load, − feed-in).",
  },

  scen: {
    heading: "Scenarios", none: "No scenarios saved yet.",
    save: "Save", load: "Load", delete: "Delete scenario",
    saveDots: "Save scenario…", saveTitle: "Save scenario",
    loadSub: "Load scenario",
    name: "Name…", desc: "Description (optional)",
    saveHint: "Save the current setup (grid, loads, batteries, meters, DERs, time) as a scenario:",
  },

  der: {
    apply: "Apply", start: "Start", dur: "Duration",
    pvTitle: "PV system — drag to resize (kWp)",
    evTitle: "EV charging @ {{kw}} kW — move the charge window",
  },

  sec: { overview: "Overview", readings: "Readings" },

  arch: {
    CHR01: "Couple both at Work", CHR03: "Family 1 child both at work",
    CHR05: "Family 3 children both with work", CHR07: "Single with work",
    CHR16: "Couple over 65 years", CHR30: "Single Retired Man",
    CHR22: "Single woman 1 child with work", CHR52: "Student Flatsharing",
  },

  common: { loading: "loading…", error: "error: {{msg}}" },

  tip: {
    line: "Line {{name}}", loadingPct: "loading {{v}} %", currentA: "I {{v}} A", powerKw: "P {{v}} kW",
    clickCurrent: "click → current graph", normallyOpen: " · normally open",
    slack: "Slack {{name}}", bus: "Bus {{name}}", kv: "{{v}} kV", vmPu: "U {{v}} V",
    clickNode: "click → load/gen graph",
    trafo: "Trafo {{name}}", kva: "{{v}} kVA", clickPower: "click → power graph",
    mvStation: "MV station {{name}} · {{kv}} kV · click: live values",
    cabinet: "Cable cabinet {{name}} · {{kv}} kV · click: live values",
    busMap: "Bus {{name}} · {{kv}} kV · click: live values · right-click: menu",
    lineMap: "Line {{name}} · click: live values · right-click: menu",
    trafoMap: "Trafo {{name}} · {{kva}} kVA · click: live values · right-click: menu",
    battery: "Battery · bus {{bus}}",
    metered: "📟 metered", noMeter: "no meter · unknown",
    voltA: "U {{v}} V", meterI: "I {{v}} A", meterP: "P {{v}} kW", meterQ: "Q {{v}} kvar",
  },
  pop: {
    noData: "no live data yet",
    unobserved: "no measurement — unknown",
    lineUnobserved: "lines carry no meters — unknown in this view",
    coldStart: "standard meter: waiting for the first 15-min window",
    slack: "grid connection (slack)",
    exchange: "exchange",
    u: "U", loading: "loading", losses: "losses",
    length: "length", rating: "rating",
    line: "line", trafo: "trafo",
    openTie: "tie open",
  },
  map: {
    dark: "🌙 Dark", light: "☀ Light", lineLoading: "line loading", busVolt: "bus voltage Δ",
    layerMv: "MV", layerLv: "LV", layerAll: "All",
    layerMvTitle: "Medium voltage only", layerLvTitle: "Low voltage only",
    layerAllTitle: "Both voltage levels",
  },
};

const de: typeof en = {
  nav: { grid: "Netz", loads: "Lasten", live: "Live" },
  app: { defaultGrid: "{{name}} (Standard)", noGrid: "kein Netz",
         busShort: "Knoten", loadsShort: "Lasten" },

  grid: {
    title: "Netz erzeugen",
    subtitle: "Synthetische deutsche Verteilnetze (ding0) auf realer Geographie — Typ, Größe und Gebietscharakter wählen.",
    voltageLevel: "Spannungsebene", mv: "Mittelspannung (20 kV)", lv: "Niederspannung (0,4 kV)",
    areaCharacter: "Gebietscharakter",
    rural: "Ländlich", suburban: "Vorstädtisch", urban: "Städtisch", user: "Eigene",
    ieee: "IEEE", cigre: "CIGRE", kerber: "Kerber",
    ruralHint: "geringe Dichte, lange Stränge", suburbanHint: "mittlere Dichte", urbanHint: "hohe Dichte, kompakt",
    userHint: "mit gridedit gezeichnete Netze",
    approxNodes: "ca. Knoten",
    noMatch: "Noch kein {{voltage}}-{{character}}-Netz in der Bibliothek — andere Kombination versuchen.",
    adjust: "Regler anpassen, um ein Netz zu wählen.",
    building: "Netz wird erstellt…",
    transformer: "Transformator: {{kva}} kVA",
    notes: "{{count}} Importhinweis(e)",
    configure: "Lasten konfigurieren →",
    failed: "Netze konnten nicht geladen werden:", loadingLib: "Netzbibliothek wird geladen…",
    noLibrary: "Keine Netzbibliothek auf dem Server gefunden.",
    runScript: "scripts/build_grid_library.py ausführen, um eine aus dem OEP zu erzeugen.",
    nodes: "{{count}} Knoten",
    kBuses: "Knoten", kLines: "Leitungen", kTrafos: "Trafos", kLoads: "Lasten",
    ruralAdj: "Ländliches", suburbanAdj: "Vorstädtisches", urbanAdj: "Städtisches",
    nameMv: "{{adj}} MS-Netz · Bezirk {{district}}",
    nameLv: "{{adj}} NS-Netz · {{lvid}}",
  },

  loads: {
    title: "Lasten erzeugen",
    subtitle: "Netz {{grid}}. Profile sind echte LPG-Haushalte, auf die Lasten verteilt.",
    noLpg: "Keine LPG-Bibliothek auf dem Server — Sie können mit synthetischen Platzhalter-Lasten fortfahren.",
    archetypes: "Haushalts-Archetypen ({{count}} ausgewählt)",
    kwhYr: "{{kwh}} kWh/Jahr",
    mode: "Zuweisungsmodus", roundRobin: "Reihum (gleichmäßig)", random: "Zufällig (mit Seed)",
    seed: "Zufalls-Seed", scale: "Skalierungsfaktor ({{scale}}×)", pf: "Leistungsfaktor ({{pf}})",
    evPen: "🔌 E-Auto-Anteil ({{pct}} %)",
    evHint: "Anteil der Haushalte, die abends ein E-Auto laden (ungesteuert).",
    wallbox: "Wallbox-Leistung", ev37: "3,7 kW (langsam)", ev11: "11 kW (Wallbox)", ev22: "22 kW (schnell)",
    evMix: "Zufall: 3,7 / 11 / 22 kW",
    pvSizing: "PV-Auslegung",
    pvEqual: "Gleiche Leistung (kWp-Regler)",
    pvMix: "Zufällige Leistung und Verteilung",
    pvPen: "☀️ PV-Anteil ({{pct}} %)", pvSize: "PV-Größe pro Haushalt ({{kwp}} kWp)",
    preview: "Vorschau", applyRun: "Übernehmen & starten →", placeholder: "Platzhalter",
    placeholderHint: "LPG überspringen, Platzhalter-Lasten verwenden",
    aggregate: "Aggregierte Tageslast",
    previewHint: "„Vorschau“ klicken, um die Netto-Lastkurve des Strangs zu simulieren.",
    kLoads: "Lasten", kEvs: "E-Autos", kPv: "PV-Anlagen", kNetPeak: "Leistungsspitze", kMinNet: "Netto-Min.",
    netDesc: "Netto-Strangbedarf (durchgezogen) über 24 h, kVA",
    netDescPv: " — gestrichelt = Bruttolast vor PV",
    reverseFlow: " Netto wird negativ → Rückspeisung mittags (PV-Einspeisung).",
    archUsed: "Verwendete Archetypen: {{list}}.",
  },

  live: {
    map: "Karte", schematic: "Schema", values: "Werte", streets: "Straßen", reset: "Ansicht zurücksetzen",
    mapTitle: "Echte OpenStreetMap-Karte an den Koordinaten des Netzes",
    valuesTitle: "Live-Strom auf Leitungen und Spannung/Leistung an Knoten anzeigen (SCADA-Stil)",
    day: "Tag {{day}} · {{time}}", notConverged: " · nicht konvergiert",
    gridInfo: "{{name}} · {{buses}} Knoten · ws {{ws}}",
    vminmax: "U min / max", maxLine: "max. Leitungsauslastung", maxTrafo: "max. Trafo-Auslastung",
    totalLoad: "Gesamtlast", generation: "Erzeugung", slack: "Bezug (Netz)",
    losses: "Verluste", solveTime: "Rechenzeit", na: "k. A.",
    selectHint: "Element anklicken für Aktionen (Verlauf, Batterie, Messung). Strg/⌘+Klick heftet den Tagesverlauf direkt an.",
    legLow: "<50 %", legMed: "<80 %", legHigh: "<100 %", legOver: "Überlast",
    underV: "Unterspg.", okV: "ok", overV: "Überspg.",
    mapHint: "Leitungen grün→rot nach Auslastung; Knoten nach Spannungs-Δ gerötet. Scrollen zum Zoomen, ziehen zum Verschieben.",
    schematicHint: "Leitungsbreite ∝ Strom. Scrollen zum Zoomen, ziehen zum Verschieben.",
    play: "▶ Start", pause: "⏸ Pause",
    time: "Zeitpunkt: {{time}} Uhr", dayLabel: "Tag", dayTitle: "Realer PV-Tag (Messdaten)",
    stepDur: "Schrittdauer", stepDurTitle: "Realzeit pro Simulationsschritt",
    failedNet: "Netz konnte nicht geladen werden:", loadingNet: "Netz wird geladen…",
    reveal: "Lastfluss an", revealOff: "Lastfluss aus",
    revealTitle: "Echten Lastfluss ein-/ausblenden (aus = nur platzierte Messungen zeigen Werte)",
    observedNote: "Nur gemessene Werte — aus platzierten Messungen.",
    truthHidden: "Wahrheit vom Server verborgen (strikte Beobachtbarkeit).",
    measuredVmm: "gemessen U min / max", measuredTrafo: "gemessen max Trafo",
    measuredLoad: "gemessene Last (Σ)", coverage: "Messabdeckung", groundTruth: "Wahrheit",
    estimate: "Schätzung", estimateTitle: "WLS-Zustandsschätzung aus den platzierten Messungen + Netzmodell",
    estCaption: "Zustandsschätzung", estErrV: "Schätzfehler max |ΔU|",
    estSolve: "Rechenzeit Schätzung",
  },

  graph: { uhr: "{{time}} Uhr", est: "Schätzung", meas: "Messung" },
  rec: {
    heading: "Daten-Export",
    exportDays: "Tage exportieren",
    exportDaysDots: "Tage exportieren…",
    exportTitle: "Tage exportieren",
    exportStart: "Export starten",
    days: "Anzahl Tage",
    estimate: "Zustandsschätzung mitrechnen",
    exportHint: "Simuliert die Tage offline mit der aktuellen Konfiguration — deutlich schneller als die Live-Uhr. Mit Schätzung im TAF-9/10/14-Modus dauert es entsprechend länger.",
    estimateTitle: "Zustandsschätzung mitrechnen (auf TAF-9/10/14-Setups langsamer)",
    exportRunning: "Bulk-Export läuft",
    expChip: "Export {{pct}} %",
    cancel: "Abbrechen",
    record: "Live-Sitzung aufzeichnen",
    recordStop: "Aufzeichnung stoppen",
    recChip: "REC {{n}}",
    recordings: "Aufzeichnungen",
    none: "noch keine Aufzeichnungen",
    delete: "Aufzeichnung löschen",
    steps: "Schritte",
    error: "Fehler",
  },
  view: {
    noMeter: "keine Messung an diesem Element — Messgerät platzieren, um Messwerte zu sehen",
    noLineMeter: "Leitungen tragen keine Messgeräte — Ströme erscheinen nur im Lastfluss oder in der Schätzung",
  },
  ctrl: {
    title: "Überlast-Regler",
    limit: "Grenze",
    limitTitle: "Drosselt, solange die Auslastung im Regelbereich über dieser Grenze liegt",
    evF: "E-Auto-Ladung auf",
    pvF: "PV-Einspeisung auf",
    active: "regelt (Auslastung über Grenze)",
    idle: "bereit — keine Überlast",
    seen: "sieht Auslastung",
    srcMeter: "gemessen",
    srcEst: "geschätzt",
    blind: "blind — keine Messdaten (Messungen platzieren)",
  },
  axis: {
    time: "Zeit / h",
    power: "Leistung / kW",
    apparent: "Leistung / kVA",
    voltage: "Spannung / V",
    current: "Strom / A",
    soc: "Ladezustand / %",
    price: "Preis / €/MWh",
  },

  mbar: {
    netz: "Netz",
    datei: "Datei",
    segTruth: "Lastfluss",
    segObserved: "Gemessen",
    segEst: "Schätzung",
    estConfigDots: "Schätz-Richtlinie…",
    sim: "Simulation",
    view: "Ansicht",
    meas: "Messungen",
    est: "Schätzung",
    help: "Hilfe",
    netzStudio: "Netz & Lasten…",
    toLive: "Live-Ansicht",
    tabsHdr: "Arbeitsbereich",
    tabLive: "Live",
    tabStudio: "Netz & Lasten",
    activeHdr: "Aktiv",
    start: "Start",
    pause: "Pause",
    saved: "Szenario gespeichert",
    viewOnlyLive: "Wirkt in der Live-Ansicht.",
    displayHdr: "Darstellung",
    sightHdr: "Sicht",
    sightTruth: "Lastfluss (Wahrheit)",
    sightObserved: "Gemessen",
    taf9Sub: "U/P/Q/I minütlich",
    taf7Sub: "15-min-Mittel P",
    manual: "Benutzerhandbuch (PDF)",
    source: "Quellcode auf GitHub",
  },
  netz: {
    step1: "1 · Netz wählen",
    step2: "2 · Lasten & Erzeugung",
    step3: "3 · Prüfen & starten",
    import: "Netz importieren…",
    imported: "Importiert: {{name}}",
    importErr: "Import fehlgeschlagen:",
    refnets: "Referenznetze (IEEE · CIGRE · Kerber)",
    library: "Bibliothek",
    own: "Eigene Netze (gridedit)",
    noOwn: "noch keine — aus gridedit exportieren oder Datei importieren",
    pickHint: "Links ein Netz wählen oder importieren.",
    kTrafo: "Transformator",
    kHouseholds: "Haushalte",
    kTrafoPeak: "max. Trafo-Auslastung",
    mfh: "Mehrfamilienhäuser (3–6 Haushalte je Gebäude)",
    mfhHint: "Wirkt in vorstädtischen und städtischen Netzen — je Gebäude werden mehrere Lastprofile addiert.",
    mfhUsed: "{{mfh}} Mehrfamilienhäuser · {{hh}} Haushalte gesamt",
    trafoLine: "Gestrichelte Linie: Trafo-Bemessungsleistung {{kva}} kVA.",
    noTrafoRating: "Für dieses Netz ist keine Trafo-Nennleistung bekannt.",
    advanced: "Erweitert (Archetypen, Seed, Skalierung)",
  },
  est: {
    tab: "Schätzung",
    tabTitle: "Konfigurieren, was die Zustandsschätzung verwenden darf",
    heading: "Richtlinie der Zustandsschätzung",
    intro: "Worauf vertraut die WLS-Schätzung des Netzbetreibers? Die Vorgaben entsprechen realer VNB-Praxis.",
    pseudoGroup: "Pseudomesswerte",
    pvPseudo: "PV-Pseudomesswerte verwenden",
    pvPseudoHint: "Üblich AUS: Anlagen sind unterschiedlich ausgerichtet, einem pauschalen Tagesgang wird nicht vertraut. Die Anlagengröße weitet weiterhin die Unsicherheit.",
    evPseudo: "E-Auto-Pseudomesswerte verwenden",
    evPseudoHint: "Üblich AUS: Ladevorgänge sind stochastisch, es gibt kein Standardprofil.",
    zeroInj: "Nullinjektion nutzen (Muffen/Kabelverteiler)",
    zeroInjHint: "Strukturwissen: An Muffen und Verteilern wird nichts verbraucht. Ohne dieses Wissen kann das System unbeobachtbar werden.",
    basisGroup: "Lastannahme (Haushalte)",
    basisProfile: "Echte Tagesmittel (idealisiert)",
    basisSlp: "Standardlastprofil (einheitlich)",
    basisProfileHint: "Das reale Tagesmittel jedes Kunden — mehr Wissen, als ein VNB tatsächlich hat.",
    basisSlpHint: "Jeder Haushalt gleich angenommen (Jahresverbrauch unten) — realistisches VNB-Wissen.",
    slpKwh: "Angenommener Jahresverbrauch (kWh)",
    stdPct: "Pseudo-Unsicherheit: ±{{pct}} % der Tagesspitze",
    stdHint: "Wie stark die Schätzung den Lastannahmen gegenüber den Messungen vertraut.",
    hierGroup: "Vertikale Schätzung (MS/NS)",
    hierAuto: "Automatisch (hierarchisch auf Bezirken)",
    hierOn: "Hierarchisch (Zellen → MS-Ebene)",
    hierOff: "Monolithisch (eine WLS über alles)",
    hierHint: "Hierarchisch = jede Ortsnetz-Zelle schätzt lokal; ihr Randfluss speist die MS-Schätzung — so schichtet es ein VNB tatsächlich. Wirkt nur auf Netzen mit Zellen.",
  },

  ront: {
    title: "rONT",
    tap: "Stufenstellung",
    seenV: "sieht Sammelschiene",
    band: "hält die Sammelschiene zwischen {{lo}} und {{hi}} V",
    target: "Sollwert",
    targetTitle: "Spannungs-Sollwert der Sammelschiene — der Stufensteller schaltet, bis die beobachtete Spannung im Totband liegt",
  },

  cells: {
    heading: "Zellen",
    back: "Bezirkssicht",
    hint: "Zelle anklicken: Die Karte zoomt hinein, ihre Station wird angeheftet. ● grün = unauffällig, gelb = dimmt auf Signal, rot = Station überlastet, grau = kein Messwert.",
  },

  ampel: {
    heading: "Netzampel",
    coordinator: "MS-Koordinator",
    signalEv: "EV-Signal (alle Zellen)",
    signalPv: "PV-Signal (alle Zellen)",
    noCoord: "Kein MS-Koordinator platziert — dafür den UW-Bus anklicken.",
    cells: "Ortsnetz-Zellen",
    boxes: "Steuerboxen (Zellenregler)",
    dimming: "dimmen gerade",
  },
  node: {
    title: "Knoten {{name}}", power: "Leistung", voltage: "Spannung",
    none: "Keine Last oder Erzeugung an diesem Knoten.",
    residential: "Haushalt", ev: "E-Auto", pv: "PV", voltageSeries: "Spannung",
    wind: "Windkraft", biogas: "Biogas", gen: "Erzeugung",
  },
  line: { title: "Leitung {{name}}", current: "Strom", rated: "Nenn {{a}} A",
          none: "Kein Strom (Leitung außer Betrieb?)." },
  trafo: { title: "Transformator {{name}}", power: "Scheinleistung S",
           rated: "Nenn {{kva}} kVA", ratedNeg: "−{{kva}} kVA",
           none: "Keine Leistungsdaten (Transformator außer Betrieb?)." },

  bat: {
    heading: "Batterien", none: "Noch keine Batterien.",
    self: "Eigenverbrauch", peak: "Spitzenlastkappung", price: "Preis",
    busN: "Knoten {{bus}}", addHint: "Einen einzelnen Knoten oder den Transformator wählen, um dort eine Batterie hinzuzufügen.",
    apply: "OK", sizeTitle: "Batteriegröße (Energie / Leistung)",
    add: "＋ Batterie @ Knoten {{bus}}", rowTitle: "Täglicher Ladezustand / Leistung",
    title: "Batterie Knoten {{bus}} · {{mode}}",
    soc: "Ladezustand", pwr: "Laden (+) / Entladen (−)", dayahead: "Day-Ahead-Preis",
    sell: "{{v}} (Verkauf)", buy: "{{v}} (Kauf)",
    noData: "Keine Leistungsdaten (Batterie außer Betrieb?).",
  },

  meas: {
    heading: "Messungen", none: "Keine Messungen platziert — das Netz ist unbeobachtbar.",
    smartMeter: "Smart Meter", trafoMeter: "Trafo-Messung",
    node: "Knoten {{bus}}", trafo: "Transformator {{trafo}}",
    coverage: "Abdeckung", coverageVal: "{{nodes}}/{{totalNodes}} Knoten · {{trafos}}/{{totalTrafos}} Trafos",
    addNodeHint: "Einen Knoten oder Transformator wählen, um dort eine Messung zu platzieren.",
    addNode: "📟 Messung @ Knoten {{bus}}", addTrafo: "📟 Messung @ Transformator {{trafo}}",
    remove: "Messung entfernen",
    modeFull: "TAF9/10/14", modeStd: "TAF7",
    modeFullTitle: "TAF 9/10/14 — Netzzustandsdaten: Spannung, P, Q und Strom im Minutentakt",
    modeStdTitle: "TAF 7 — Zählerstandsgang: nur 15-Minuten-Mittelwert der Wirkleistung",
    deviceModeTitle: "Messung dieses Geräts: TAF 9/10/14 = U/P/Q/I minütlich, TAF 7 = nur 15-min-Mittel P",
    perDeviceHint: "Gilt für alle platzierten Geräte und als Vorgabe für neue — einzelne Geräte stellst du in ihrer Sektion um.",
    presets: "Schnell", presetAllNodes: "Alle Knoten", presetSubstations: "Stationen",
    presetAllTrafos: "Alle Trafos", clear: "Alle löschen",
    presetDigitalStations: "Digitale Ortsnetzstationen",
    presetDigitalStationsSub: "eine Stationsmessung je Zelle ({{n}} Zellen)",
    strict: "Strikter Modus — der Server verbirgt die Wahrheit.",
    unknown: "unbekannt",
  },

  menu: {
    graph: "Tagesverlauf", addMeter: "Messung platzieren", removeMeter: "Messung entfernen",
    addBattery: "Batterie hinzufügen", addBatteryTrafo: "Batterie @ NS-Sammelschiene",
    addController: "Regler platzieren", addControllerTrafo: "Netzregler @ Station",
    addControllerCell: "Steuerbox (Zellenregler) platzieren",
    addControllerMv: "Netzampel-Koordinator (MS) platzieren",
    removeController: "Regler entfernen",
    addRont: "rONT aktivieren (Stufensteller)",
    removeRont: "rONT entfernen",
    removeBattery: "Batterie entfernen",
    addPv: "PV-Anlage hinzufügen", addEv: "E-Auto-Ladung hinzufügen",
    removePv: "PV-Anlage entfernen", removeEv: "E-Auto-Ladung entfernen",
    addExt: "Externe Quelle anbinden", removeExt: "Externe Quelle entfernen",
  },

  ext: {
    title: "Externe Quelle",
    applied: "angewandter Wert",
    age: "letztes Telegramm",
    ageVal: "vor {{s}} s",
    never: "noch kein Telegramm",
    stale: "Quelle stumm seit {{s}} s — {{policy}}",
    staleNever: "noch kein Telegramm empfangen — {{policy}}",
    policyHold: "der letzte Wert wird gehalten",
    policyZero: "der Wert fällt auf 0",
    hold: "bei Timeout halten",
    zero: "bei Timeout auf 0",
    bound: "Wertgrenze",
    history: "empfangen",
    feedHint: "Live-Werte einspeisen: PUT /api/ext/{{id}}/value mit {\"p_kw\": …} (vorzeichenbehaftet: + Last, − Einspeisung).",
  },

  scen: {
    heading: "Szenarien", none: "Noch keine Szenarien gespeichert.",
    save: "Speichern", load: "Laden", delete: "Szenario löschen",
    saveDots: "Szenario speichern…", saveTitle: "Szenario speichern",
    loadSub: "Szenario laden",
    name: "Name…", desc: "Beschreibung (optional)",
    saveHint: "Aktuelles Setup (Netz, Lasten, Batterien, Messungen, Anlagen, Zeitpunkt) als Szenario sichern:",
  },

  der: {
    apply: "OK", start: "Beginn", dur: "Dauer",
    pvTitle: "PV-Anlage — Größe anpassen (kWp)",
    evTitle: "E-Auto-Ladung @ {{kw}} kW — Ladefenster verschieben",
  },

  sec: { overview: "Übersicht", readings: "Messwerte" },

  arch: {
    CHR01: "Paar, beide berufstätig", CHR03: "Familie, 1 Kind, beide berufstätig",
    CHR05: "Familie, 3 Kinder, beide berufstätig", CHR07: "Single, berufstätig",
    CHR16: "Paar über 65 Jahre", CHR30: "Alleinstehender Rentner",
    CHR22: "Alleinerziehende, 1 Kind, berufstätig", CHR52: "Studenten-WG",
  },

  common: { loading: "lädt…", error: "Fehler: {{msg}}" },

  tip: {
    line: "Leitung {{name}}", loadingPct: "Auslastung {{v}} %", currentA: "I {{v}} A", powerKw: "P {{v}} kW",
    clickCurrent: "Klick → Stromverlauf", normallyOpen: " · normal offen",
    slack: "Slack {{name}}", bus: "Knoten {{name}}", kv: "{{v}} kV", vmPu: "U {{v}} V",
    clickNode: "Klick → Last/Erzeugung",
    trafo: "Trafo {{name}}", kva: "{{v}} kVA", clickPower: "Klick → Leistungsverlauf",
    mvStation: "MS-Station {{name}} · {{kv}} kV · Klick: Live-Werte",
    cabinet: "Kabelverteiler {{name}} · {{kv}} kV · Klick: Live-Werte",
    busMap: "Knoten {{name}} · {{kv}} kV · Klick: Live-Werte · Rechtsklick: Menü",
    lineMap: "Leitung {{name}} · Klick: Live-Werte · Rechtsklick: Menü",
    trafoMap: "Trafo {{name}} · {{kva}} kVA · Klick: Live-Werte · Rechtsklick: Menü",
    battery: "Batterie · Knoten {{bus}}",
    metered: "📟 gemessen", noMeter: "keine Messung · unbekannt",
    voltA: "U {{v}} V", meterI: "I {{v}} A", meterP: "P {{v}} kW", meterQ: "Q {{v}} kvar",
  },
  pop: {
    noData: "noch keine Live-Daten",
    unobserved: "keine Messung — unbekannt",
    lineUnobserved: "Leitungen tragen keine Messgeräte — in dieser Sicht unbekannt",
    coldStart: "Standard-Zähler: warte auf das erste 15-min-Fenster",
    slack: "Netzanschluss (Slack)",
    exchange: "Austausch",
    u: "U", loading: "Auslastung", losses: "Verluste",
    length: "Länge", rating: "Nennleistung",
    line: "Leitung", trafo: "Trafo",
    openTie: "Trennstelle offen",
  },
  map: {
    dark: "🌙 Dunkel", light: "☀ Hell", lineLoading: "Leitungs­auslastung", busVolt: "Spannungs-Δ",
    layerMv: "MS", layerLv: "NS", layerAll: "Alle",
    layerMvTitle: "Nur Mittelspannung", layerLvTitle: "Nur Niederspannung",
    layerAllTitle: "Beide Spannungsebenen",
  },
};

i18n.use(initReactI18next).init({
  resources: { en: { translation: en }, de: { translation: de } },
  lng: "de",
  fallbackLng: "en",
  interpolation: { escapeValue: false },
});

export default i18n;
