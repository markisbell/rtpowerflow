...
/** Top tab "Schätzung": a context-menu style dropdown that configures what
 *  the WLS state estimation may use. Defaults mirror real DSO practice —
 *  no PV / EV pseudo-measurements. Every change POSTs immediately; the
...
export default function EstimationMenu() {
  ...
  const check = (key: "pv_pseudo" | "ev_pseudo" | "zero_injection",
                 label: string, hint?: string) => (
    <label className="est-row" title={hint}>
  ...
  return (
    <div className="est-menu">
      <button className={open ? "on" : ""} onClick={() => setOpen(!open)}
              title={t("est.tabTitle")}>
        🧮 {t("est.tab")} ▾
      </button>
      {open && (
        <>
          <div className="est-overlay" onClick={() => setOpen(false)} />
          <div className="est-panel">
            <div className="est-head">{t("est.heading")}</div>
            <div className="est-note">{t("est.intro")}</div>

            <div className="est-group">{t("est.pseudoGroup")}</div>
            {check("pv_pseudo", t("est.pvPseudo"), t("est.pvPseudoHint"))}
            <div className="est-hint">{t("est.pvPseudoHint")}</div>
            {check("ev_pseudo", t("est.evPseudo"), t("est.evPseudoHint"))}
            <div className="est-hint">{t("est.evPseudoHint")}</div>
            {check("zero_injection", t("est.zeroInj"), t("est.zeroInjHint"))}
