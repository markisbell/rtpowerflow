...
\begin{description}[nosep]
  \item[\ui{Netz}] öffnet die kombinierte Ansicht \ui{Netz \&
    Lasten…} --- Netz wählen oder importieren, Lasten konfigurieren,
    Trafo-Auslastung prüfen, starten; darunter steht das aktuell
    geladene Netz.
...
\ui{Netz} $\rightarrow$ \ui{Netz \& Lasten…} öffnet die kombinierte
Ansicht. Die linke Spalte listet alle verfügbaren Netze:
...
Rechenzeit & 8{,}2 ms & Lastfluss-Dauer dieses Schrittes \\
\bottomrule
\end{tabular}
\end{center}
...
\glqq gemessene Last ($\Sigma$) 10{,}6~kW\grqq{} --- die Wahrheit im
selben Moment: 154{,}8~kW. Die 10{,}6~kW sind schlicht die Summe der
vier gemessenen Hausanschlüsse. Wer nur auf Messwerte schaut, sieht
7\,\% des Geschehens; die \glqq gemessen max Trafo 67{,}6\,\%\grqq{}
...
Die Schätzung rechnet dabei im \emph{Messraster}: Im Modus
\ui{TAF 9/10/14} liefern die Geräte minütlich neue Werte, also wird
minütlich neu geschätzt. Im Modus \ui{TAF 7} kommt nur alle
15~Minuten ein neuer Lastgangwert --- eine neue Schätzung entsteht
folglich nur an den Viertelstundengrenzen, dazwischen hält sie den
letzten Stand. Auf großen Netzen wird zusätzlich automatisch
gedrosselt (die Schätzung rechnet dann z.\,B. nur jeden dritten
Schritt), damit die Simulation flüssig bleibt.
...
\begin{figure}[htbp]
  ...
  \caption{Ansicht \ui{Schätzung}: Aus fünf Messungen und dem
    ...
    den Schätzfehler gegen die Wahrheit aus (hier 0{,}7~V).}
...
\begin{enumerate}[nosep]
  \item \textbf{Nur Stations-Trafo} (Preset \ui{Stationen}): Die
    ...
    verfehlt, das höchstbelastete Kabel wird nur ungefähr erkannt.
  \item \textbf{+ 3 Smart Meter} an den Strangenden: Der Fehler
    fällt auf \textbf{0{,}7~V} (Screenshot) --- die Meter an den
    kritischen Enden \glqq spannen\grqq{} das Spannungsprofil auf.
    Auch die geschätzte maximale Leitungsauslastung (52\,\%) liegt
...
\begin{figure}[htbp]
  ...
  \caption{Netzregler an der Station im Referenzszenario~3 (12:52
    Uhr): Die Regler-Sektion zeigt \ui{sieht Auslastung}
    87{,}5\,\% (geschätzt) --- über der Grenze von 100\,\% hat er
    die \ui{PV-Einspeisung auf} 80\,\% gedrosselt, die E-Auto-Ladung
    blieb unangetastet. Die wahre maximale Leitungsauslastung ist
    dadurch von $\approx$110 auf 87\,\% gefallen; das mittlere
...
\ui{Simulation} unter \ui{Daten-Export}
(Abbildung~\ref{fig:export}) und erzeugen identisch aufgebaute
Pakete:

\begin{description}
  \item[\ui{Tage exportieren}] --- der Standardweg. Die gewünschte
    Anzahl Tage wird \emph{offline} so schnell wie möglich
    durchsimuliert, ohne auf die beschleunigte Uhr zu warten: zwei
    volle Tage des Vorstadtnetzes rechnen in etwa einer Minute,
    live wären es 48 Minuten. Gerechnet wird die \emph{echte}
    Live-Physik der aktuellen Konfiguration: Batterien starten bei
    50\,\% und integrieren über Mitternacht hinweg, Überlast-Regler
    regeln im geschlossenen Kreis, bei geladenen PV-Messtagen ist
    jeder exportierte Tag ein anderer realer Solartag. Das kleine
    Häkchen neben der Tageszahl entscheidet, ob die
    Zustandsschätzung mitläuft --- im Modus TAF~9/10/14 rechnet sie
    minütlich und dominiert dann die Exportdauer deutlich. Während des Exports zeigt die Zeile
    Fortschritt und Restzeit (\ui{Abbrechen} behält das bereits
    Gerechnete); die Simulation läuft währenddessen normal weiter,
    rechts in der Menüleiste erscheint ein Fortschritts-Chip.
...
Fertige Pakete erscheinen darunter als Liste: Ein Klick lädt das
ZIP, das $\times$ daneben löscht. Auf dem Server liegen die Pakete
unter \texttt{data/recordings/}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{ui-export}
  \caption{\ui{Daten-Export} im Menü \ui{Simulation}: oben der
    laufende Tages-Export mit Fortschritt und \ui{Abbrechen},
    darunter der Aufzeichnungs-Umschalter und die fertigen Pakete
    zum Download; rechts in der Menüleiste der Fortschritts-Chip,
    der auch bei geschlossenem Menü sichtbar bleibt.}
  \label{fig:export}
\end{figure}
