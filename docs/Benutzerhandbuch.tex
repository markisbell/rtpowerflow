...
Die Oberfläche führt in drei Schritten durch die Simulation. Die
Navigation oben zeigt \ui{Netz}, \ui{Lasten} und \ui{Live}. Alle
Beispiele dieses Kapitels stammen aus \emph{einem} durchgängigen
Arbeitsgang mit demselben Netz, sodass sich die Zahlen von Schritt zu
Schritt wiederfinden lassen.

\section{Schritt 1: Netz erzeugen}

Auf der Seite \ui{Netz erzeugen} wird das zu simulierende Netz
gewählt. Die Netzbibliothek enthält synthetische deutsche
...
Auf der Seite \ui{Lasten erzeugen} werden den 89 Hausanschlüssen
...
\begin{itemize}[nosep]
  \item \ui{Start} / \ui{Pause} halten die Simulation an und setzen
    sie fort --- praktisch, um einen Zustand in Ruhe zu besprechen.
...
Zwei Gerätetypen stehen zur Verfügung (Sektion \ui{Messungen} im
Seitenpanel oder per Element-Menü):
...
Schnell-Aktionen: \ui{Alle Knoten}, \ui{Stationen} (nur
Stations-Trafos), \ui{Alle Trafos}, \ui{Alle löschen}. Die
\ui{Abdeckung} (gemessene Knoten/Trafos) wird laufend angezeigt
(Abbildung~\ref{fig:beobachtet}).

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{ui-beobachtet}
  \caption{Beobachtete Ansicht (\ui{Lastfluss aus}): nur vier Knoten
...
Über die Schaltflächen der Live-Ansicht wählt man:

\begin{description}
  \item[\ui{Lastfluss an} (Wahrheit)] Der echte Lastfluss wird
    vollständig eingeblendet --- die Vergleichs- und Lehransicht.
  \item[\ui{Lastfluss aus} (nur beobachtet)] Werte nur an platzierten
...
\begin{itemize}[nosep]
  \item \ui{Speichern} in der Sektion \ui{Szenarien} legt das aktuelle
    Setup unter einem Namen ab (optional mit Beschreibung).
  \item \ui{Laden} spielt das Rezept vollständig ab und startet am
