...
\begin{description}
  \item[Wahrheit] Der vollständige, physikalisch gelöste Lastfluss ---
    das, was im Netz \emph{tatsächlich} passiert. In der Realität kennt
    ihn niemand.
  \item[Beobachtung] Nur das, was platzierte Messgeräte (Smart Meter,
    Trafo-Messungen) tatsächlich erfassen. Ein frisches Netz ist
    nahezu unbeobachtbar --- genau wie ein reales Ortsnetz.
...
\begin{description}[nosep]
  \item[\ui{Netz}] öffnet den Netzkatalog (\ui{Netz wählen…}) und die
    Lastkonfiguration (\ui{Lasten konfigurieren…}); darunter steht
    das aktuell geladene Netz.
  \item[\ui{Simulation}] Start/Pause sowie die komplette
    Szenarien-Verwaltung (Kapitel~\ref{ch:scenarios}).
  \item[\ui{Ansicht}] Darstellung (Karte/Schema/Werte) und Sicht
    (Wahrheit / Nur beobachtet / Schätzung,
    Kapitel~\ref{ch:observability}).
  \item[\ui{Messungen}] Mess-Presets, TAF-Modus und die aktuelle
    Abdeckung.
  \item[\ui{Schätzung}] die Richtlinie der Zustandsschätzung
    (Kapitel~\ref{ch:observability}).
...
\begin{figure}[htbp]
  ...
  \caption{Die Menüleiste über der Live-Ansicht, hier mit geöffnetem
    \ui{Ansicht}-Menü: Darstellung und Sicht als Auswahlgruppen. Die
...
Lasten erzeugen, live beobachten. Alle Beispiele dieses Kapitels
stammen aus \emph{einem} durchgängigen Arbeitsgang mit demselben
Netz, sodass sich die Zahlen von Schritt zu Schritt wiederfinden
lassen.

\section{Schritt 1: Netz erzeugen}

\ui{Netz} $\rightarrow$ \ui{Netz wählen…} öffnet den Netzkatalog.
Die Netzbibliothek enthält synthetische deutsche
Verteilnetze (ding0) auf realer Geographie --- die Topologien sind
künstlich, aber nach den Regeln realer Netzplanung erzeugt und an
echten Orten verankert, sodass die Karte echte Straßen und Gebäude
zeigt. Drei Filter grenzen die Auswahl ein:

\begin{description}
  \item[Spannungsebene] \ui{Mittelspannung (20 kV)} oder
    \ui{Niederspannung (0,4 kV)}.
  \item[Gebietscharakter] \ui{Ländlich}, \ui{Vorstädtisch},
    \ui{Städtisch} oder \ui{Eigene}. Der Charakter prägt die
    Netzform: ländliche NS-Netze sind reine Strahlennetze mit langen
    Ausläufern (spannungskritisch), vorstädtische Netze offene Ringe
    in radialem Betrieb, städtische Netze dicht vermascht
    (auslastungskritisch). \ui{Eigene} listet mit dem Editor
    \emph{gridedit} selbst gezeichnete Netze
    (Kapitel~\ref{ch:gridedit}).
  \item[ca.\ Knoten] gewünschte Netzgröße.
\end{description}

Die Trefferliste zeigt passende Netze; rechts erscheinen die
Kenndaten des gewählten Netzes samt eventueller Importhinweise.
\ui{Lasten konfigurieren} übernimmt die Auswahl
(Abbildung~\ref{fig:netz}).

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{ui-netz}
  \caption{Schritt 1 --- \ui{Netz erzeugen}: Filter (Spannungsebene,
    Gebietscharakter, Knotenzahl), Trefferliste aus der Netzbibliothek
    und rechts die Kenndaten des gewählten Netzes.}
  \label{fig:netz}
\end{figure}

\begin{beispiel}[Das Netz der folgenden Kapitel]
Mit den Filtern \ui{Niederspannung} + \ui{Vorstädtisch} und dem
Regler bei ca.\ 100 Knoten fällt die Wahl auf das Netz
\emph{Vorstädtisches NS-Netz~·~273437}: 134~Knoten, 132~Leitungen,
1~Ortsnetztransformator, 89~Hausanschlüsse. Die Importhinweise
melden u.\,a., auf welche Standardgröße der Transformator dimensioniert
wurde. Ein Dorf dieser Größe ist ideal, um Spannungsband und
Leitungsauslastung gleichzeitig zu beobachten.
\end{beispiel}

\emph{Besonderheit Mittelspannung:} Ein MS-Netz aus der Bibliothek
lädt automatisch die straßengerouteten NS-Ortsnetze desselben
Bezirks mit --- es entsteht ein zusammenhängendes MS+NS-Gebiet mit
mehreren hundert Knoten über zwei Spannungsebenen. NS-Ortsnetze ohne
Straßenmodell bleiben als \emph{Summenlast} an ihrer Ortsnetzstation:
ein einzelner Lastpunkt, der das ganze dahinterliegende Dorf
vertritt. Auf der Live-Karte lässt sich dann mit dem Ebenenumschalter
\ui{MS}/\ui{NS}/\ui{Alle} filtern.

\section{Schritt 2: Lasten erzeugen}

In der Lastkonfiguration (\ui{Lasten konfigurieren} im Katalog oder
im \ui{Netz}-Menü) werden den 89 Hausanschlüssen
realistische Tagesprofile zugewiesen. Grundlage sind Profile aus dem
LoadProfileGenerator (LPG), der das Verhalten konkreter Haushalte
minutengenau simuliert --- Kochen, Waschmaschine, Fernseher,
Anwesenheit. Die Bibliothek enthält u.\,a.\ folgende Archetypen
(Jahresverbräuche der hinterlegten Profile):
...
\begin{itemize}[nosep]
  \item \ui{Zuweisungsmodus}: \ui{Reihum (gleichmäßig)} verteilt die
    gewählten Archetypen der Reihe nach über die Häuser;
    \ui{Zufällig (mit Seed)} würfelt --- derselbe \ui{Zufalls-Seed}
    erzeugt dabei immer dieselbe Verteilung. Das ist für Übungen
    wichtig: Alle Gruppen mit Seed~0 sehen exakt dasselbe Netz.
  \item \ui{Skalierungsfaktor}: multipliziert alle Haushaltsprofile.
    Mit 2{,}0$\times$ lässt sich ein schwach dimensioniertes Netz
    gezielt an seine Grenzen bringen, ohne die Profile zu wechseln.
  \item \ui{Leistungsfaktor} ($\cos\varphi$, Vorgabe 0{,}95):
    bestimmt die Blindleistung, die zu jeder Wirklast gerechnet wird.
  \item \ui{E-Auto-Anteil} und \ui{Wallbox-Leistung} (3{,}7 / 11 /
    22~kW): der gewählte Anteil der Haushalte lädt abends
...
\ui{Vorschau} simuliert die aggregierte Netto-Tageslast des Strangs
vor dem Start: durchgezogen der Netto-Bedarf, gestrichelt die
Bruttolast vor PV (Abbildung~\ref{fig:lasten}). \ui{Übernehmen \&
starten} lädt das Netz mit den erzeugten Profilen in die
Live-Simulation. Alternativ lässt sich der Schritt mit
...
\begin{figure}[htbp]
  ...
  \caption{Schritt 2 --- \ui{Lasten erzeugen}: links Archetypen und
    Regler (hier 40\,\% E-Auto-Anteil, 50\,\% PV), rechts die
    Vorschau der aggregierten Tageslast. Die Netto-Kurve wird mittags
    negativ --- der Strang speist zurück (PV-Einspeisung).}
  \label{fig:lasten}
\end{figure}

\begin{beispiel}[Gleichzeitigkeit, E-Autos und PV in Zahlen]
Für das 89-Häuser-Netz zeigt die Vorschau nacheinander:
\begin{itemize}[nosep]
  \item \textbf{Nur Haushalte:} Netto-Spitze \textbf{110{,}5~kW}
    gegen 18:30 Uhr. Würde man die Einzelspitzen der Haushalte
    addieren, käme ein Vielfaches heraus --- aber nie kochen alle
    gleichzeitig. Diese \emph{Durchmischung} ist der Grund, warum
    ein 400-kVA-Transformator für ein ganzes Dorf reicht.
  \item \textbf{+ 40\,\% E-Autos (11~kW):} 33 Fahrzeuge laden abends
    ungesteuert. Die Spitze steigt auf \textbf{186{,}5~kW} (+69\,\%!),
    obwohl rechnerisch $33 \times 11 = 363$~kW installiert sind ---
    die Ladefenster überlappen nur teilweise. E-Mobilität wirkt fast
    ausschließlich auf die Abendspitze.
  \item \textbf{+ 50\,\% PV (5~kWp):} 45 Anlagen mit zusammen
    225~kWp drücken die Netto-Kurve mittags auf
    \textbf{$-$177{,}6~kW} --- der Strang speist dann mehr ins
    Mittelspannungsnetz zurück, als er abends bezieht. Abendspitze
    und Mittagstal liegen zeitlich so weit auseinander, dass PV die
    E-Auto-Spitze \emph{nicht} entlastet.
\end{itemize}
...
\begin{figure}[htbp]
  ...
  \caption{Schritt 3 --- \ui{Live} zur Abendspitze (19:14 Uhr): Kabel
    färben sich nach Auslastung (hier bis 79\,\%), an den Häusern
...
\begin{itemize}[nosep]
  \item \ui{Start} / \ui{Pause} (Transportleiste unten oder Menü
    \ui{Simulation}) halten die Simulation an und setzen sie fort ---
    praktisch, um einen Zustand in Ruhe zu besprechen.
...
Anhand des Screenshots (19:14 Uhr, Abendspitze mit E-Autos):
...
\midrule
U min / max & 217{,}5 / 230 V & Tiefpunkt am Strangende: $-5{,}4\,\%$ \\
 & & (EN-50160-Band: $\pm 10\,\% \Rightarrow$ 207--253 V) \\
max.\ Leitungsauslastung & 79 \% & höchstbelastetes Kabel, noch Reserve \\
max.\ Trafo-Auslastung & 56{,}8 \% & 400-kVA-Station gut ausgelastet \\
Gesamtlast & 149{,}5 kW & Haushalte + ladende E-Autos \\
Erzeugung & 21{,}9 kW & Rest-PV der Abendsonne \\
Bezug (Netz) & 133{,}1 kW & Import aus dem MS-Netz \\
Verluste & 3{,}56 kW & $\approx 2{,}4\,\%$ der Last --- Abendspitze \\
 & & treibt die $I^2R$-Verluste \\
Rechenzeit & 8{,}5 ms & Lastfluss-Dauer dieses Schrittes \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{beispiel}[Was man um 19:14 Uhr auf der Karte sieht]
Der Hauptstrang von der Station zum dichtesten Ortsteil ist orange
(79\,\%), seine Ausläufer gelbgrün; an gut zwei Dutzend Häusern
  ...
  \item \ui{Tagesverlauf} heftet ein Diagramm des Elements als
    ...
    \begin{itemize}[nosep]
      ...
      \item \emph{Leitung}: Strom über den Tag gegen den Nennstrom
        (z.\,B. \glqq Nenn 319~A\grqq) --- Überlastphasen sind sofort
        sichtbar.
      \item \emph{Transformator}: Leistungsaustausch gegen die
        Nennleistung in beide Richtungen ($\pm$400~kVA) ---
...
zwischen 18 und 21 Uhr Richtung 218~V ab --- deutlich sichtbar über
...
\begin{description}
  ...
  \item[TAF 7] \glqq Zählerstandsgang\grqq: nur der
    ...
    Standard-Rollout tatsächlich liefert.
\end{description}

\begin{beispiel}[Warum TAF 7 Spitzen verschluckt]
Eine 22-kW-Wallbox lädt von 19:02 bis 19:12 Uhr --- zehn Minuten
volle Leistung. Im TAF-9-Modus zeigt das Smart Meter des Hauses
diese 22~kW minutengenau. Im TAF-7-Modus erscheint im Intervall
19:00--19:15 nur der Mittelwert
$22 \cdot \tfrac{10}{15} \approx 14{,}7$~kW --- ein Drittel der
Spitze ist \emph{in den Messdaten} verschwunden, im Kabel aber
natürlich geflossen. Der Umschalter \ui{TAF9/10/14}/\ui{TAF7} in
der Messungs-Sektion macht den Unterschied in allen angehefteten
Diagrammen sichtbar.
\end{beispiel}

\section{Die drei Ansichtsmodi}

Im Menü \ui{Ansicht} wählt man unter \ui{Sicht}:

\begin{description}
  \item[\ui{Wahrheit (Lastfluss)}] Der echte Lastfluss wird
    vollständig eingeblendet --- die Vergleichs- und Lehransicht.
  \item[\ui{Nur beobachtet}] Werte nur an platzierten
...
\glqq gemessene Last ($\Sigma$) 6{,}5~kW\grqq{} --- die Wahrheit im
selben Moment: 149{,}5~kW. Die 6{,}5~kW sind schlicht die Summe der
vier gemessenen Hausanschlüsse. Wer nur auf Messwerte schaut, sieht
4\,\% des Geschehens; die \glqq gemessen max Trafo 70{,}4\,\%\grqq{}
...
Messgeräten. Auf großen Netzen wird die Schätzung automatisch
gedrosselt (sie rechnet dann z.\,B. nur jeden dritten Schritt), damit
die Simulation flüssig bleibt.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{ui-schaetzung}
  \caption{Ansicht \ui{Schätzung}: Aus fünf Messungen und dem
    ...
    den Schätzfehler gegen die Wahrheit aus (hier 1{,}45~V).}
  \label{fig:schaetzung}
\end{figure}

\begin{beispiel}[Messkampagne im Experiment]
Ein lohnender Unterrichtsverlauf auf dem 134-Knoten-Netz zur
Abendspitze:
\begin{enumerate}[nosep]
  \item \textbf{Nur Stations-Trafo} (Preset \ui{Stationen}): Die
    Schätzung kennt die Gesamtbilanz, verteilt sie aber nach
    Standardannahmen --- wahre Spannungen werden auf einige Volt
    verfehlt, das überlastete Kabel wird nur ungefähr erkannt.
  \item \textbf{+ 4 Smart Meter} an den Strangenden: Der Fehler
    fällt auf \textbf{1{,}45~V} (Screenshot) --- die Meter an den
    kritischen Enden \glqq spannen\grqq{} das Spannungsprofil auf.
    Die geschätzte maximale Leitungsauslastung (97{,}4\,\%) liegt
    nun nahe an der Wahrheit.
...
\end{enumerate}
Merksatz, den das Experiment liefert: \emph{Wenige, gut platzierte
Messungen schlagen viele schlecht platzierte} --- Strangenden zuerst.
\end{beispiel}

% ===================================================================
\chapter{Anlagen: Batterien, PV, E-Autos}
\label{ch:der}
...
\emph{Wichtig:} Laufzeit-Anlagen gehören zum aktuell geladenen Netz.
Ein Netzwechsel setzt sie zurück --- wer ein Setup behalten möchte,
speichert es als Szenario (Kapitel~\ref{ch:scenarios}).
...
\begin{itemize}[nosep]
  \item Im Menü \ui{Simulation}: Namen eintippen und \ui{Speichern}
    legt das aktuelle Setup ab.
  \item Ein Klick auf einen Szenario-Eintrag im selben Menü spielt
    das Rezept vollständig ab und startet am gespeicherten Zeitpunkt;
    das $\times$ daneben löscht es.
\end{itemize}
...
\begin{beispiel}[Unterrichtsszenario]
Die Datei \texttt{pv-einspeisung-leitung-ueberlastet.json}
(im Szenarien-Ordner \texttt{data/scenarios/}) stellt mit einem
Klick eine mittägliche Leitungsüberlastung durch PV her --- 100\,\% PV-Anteil mit je 10~kWp auf einem selbst
gezeichneten Netz, Sprungziel 19:26 Uhr, Schnelldurchlauf:

\begin{lstlisting}
{
  "version": 1,
  "name": "PV-Einspeisung überlastet Leitung",
  "grid_id": "user_beispiel",
  "loadgen": {
    "archetypes": ["CHR01", "CHR03", "CHR05", "CHR07",
                    "CHR16", "CHR22", "CHR30", "CHR52"],
    "mode": "round_robin", "seed": 0, "scale": 1.0,
    "power_factor": 0.95,
    "ev_penetration": 1.0, "ev_charger_kw": 22.0,
    "pv_penetration": 1.0, "pv_kwp": 10.0
  },
  "der_ops": [],
  "batteries": [],
  "measurements": { "node_buses": [], "trafo_idxs": [],
                    "mode": "full" },
  "engine": { "day": 0, "step": 1166, "interval_seconds": 0.1 }
}
\end{lstlisting}

Zum Anpassen genügt ein Texteditor: \texttt{"pv\_kwp": 15.0}
verschärft die Überlastung, ein Eintrag in \texttt{"batteries"}
(z.\,B. \texttt{\{"bus": 1, "capacity\_kwh": 20, "power\_kw": 10,
"mode": "peak"\}}) stellt die Gegenmaßnahme gleich mit ins Rezept.
\end{beispiel}
...
\emph{ohne Neustart} unter \ui{Eigene} im Netzkatalog --- NS-Netze
mit dem gewählten Transformator, MS-Netze mit ihrer
110-kV-Einspeisung. Über \ui{Netzabschnitt laden} lassen sich
gespeicherte Netze später wieder in den Editor holen und
weiterzeichnen.
...
GET & \api{/node/\{bus\}/profiles} u.\,ä. & Tagesverläufe je Element \\
...
\begin{description}
  ...
  \item[Die Schätzung erscheint nicht] Es ist keine Messung
    platziert. Mindestens ein Smart Meter oder eine Trafo-Messung
    setzen; danach taucht der Modus \ui{Schätzung} auf.
