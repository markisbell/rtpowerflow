...
\textbf{Hinweis zur Entstehung:} Der Quellcode des
EchtzeitNetzSimulators sowie dieses Handbuch wurden mit einem
\textbf{KI-Coding-Agenten} (Claude Code, Anthropic) erstellt --- unter
menschlicher Anleitung: Anforderungen, fachliche Entscheidungen,
Review und die Live-Verifikation der Ergebnisse stammen von Menschen.
Die Lastfluss-Physik ist gegen OpenDSS und MATPOWER validiert
(Abschnitt~\ref{sec:validierung}).
Quellcode und Handbuch stehen unter der MIT-Lizenz; die mitgelieferten
Netzdaten sind aus OpenStreetMap-Daten abgeleitet
(\textcopyright{} OpenStreetMap contributors, ODbL). Projektseite:
\url{https://github.com/markisbell/rtpowerflow}}}
\end{center}
...
Ein Lehrwerkzeug ist nur so glaubwürdig wie sein Lastfluss. Die
Physik des EchtzeitNetzSimulators ist deshalb gegen zwei unabhängige,
etablierte Referenzprogramme validiert: \textbf{OpenDSS} (EPRI) und
\textbf{MATPOWER~8.1}. Verglichen wurden sowohl die klassischen
IEEE-Referenzfälle (14/30/118 Knoten, auf identischer Datenbasis:
maximale Spannungsabweichung $\le 2{,}6\cdot10^{-10}$~pu) als auch
--- entscheidender --- \emph{genau die Netze und Tagesprofile dieses
Handbuchs}: Alle drei Lehrnetze (30, 62 und 475 Knoten) wurden über
vollständige 1440-Schritt-Tage von beiden Programmen unabhängig
nachgerechnet. Die maximale Abweichung liegt bei
$\approx 5\cdot10^{-7}$~pu --- \textbf{0{,}2~Millivolt} auf der
0,4-kV-Ebene, an jedem Knoten, in jeder Minute des Tages.

\begin{beispiel}[Was das konkret bedeutet]
Die Spannungs"-überhöhung aus Referenzszenario~1 --- die
75-kWp-Anlage hebt den Bauernhof mittags auf $\approx 252$~V, haarscharf
unter die EN-50160-Grenze von 253~V --- ist keine Eigenheit dieses
Simulators: OpenDSS berechnet im Tagesmodus dieselbe Glocke
(437~V verkettet) auf $\pm 0{,}2$~mV deckungsgleich nach. Wer eine
Aussage dieses Handbuchs anzweifelt, kann sie unabhängig nachprüfen.
\end{beispiel}

Der vollständige Bericht mit Fehlertabellen, Tagesprofil-Vergleichen
und den exakten Programmversionen liegt im Repository unter
\texttt{docs/benchmarks/}; Methode und Reproduktionsanleitung (ein
Befehl: \texttt{python benchmarks/run\_all.py}) in
\texttt{docs/BENCHMARKS.md}. Die Eingaben sind als eingefrorene
Fixtures versioniert, sodass Dritte die Zahlen bitgenau
nachvollziehen können.

\section{Weiterführende Dokumentation}
\label{sec:weiterdoku}

Dieses Handbuch beschreibt die Bedienung. Für Entwicklung und
Skript-Anbindung liegen im Repository zwei englischsprachige
Dokumente unter \texttt{docs/}:

\begin{description}
  \item[\texttt{docs/API.md}] die vollständige REST-Referenz (alle
    Endpunkte mit Parametern und Request-Feldern). Sie wird aus dem
    laufenden Code generiert (\texttt{scripts/gen\_api\_doc.py}) und
    kann daher nicht veralten; die interaktive Variante ist die
    eingebaute Swagger-Oberfläche unter
    \api{http://localhost:8000/docs} (Kapitel~\ref{app:api} zeigt
    eine Auswahl mit Beispielen).
  \item[\texttt{docs/ARCHITECTURE.md}] die Systemarchitektur:
    Komponenten und Datenfluss, die drei Datenebenen als
    Kernkonzept, Modulübersicht, vertikale MS/NS-Integration und
    die zentralen Design-Entscheidungen samt Begründung.
\end{description}
...
\begin{description}[nosep]
  \item[\ui{Datei}] alles, was lädt, speichert oder exportiert:
    Szenarien speichern und laden (Kapitel~\ref{ch:scenarios})
    sowie Aufzeichnung und \ui{Daten-Export}
...
Rechts neben den Menüs sitzen zwei dauerhaft sichtbare
Umschalter: der \textbf{Arbeitsbereich} \ui{Live~/ Netz \& Lasten}
(zwischen laufender Simulation und der Netz-Konfiguration wechseln
--- wie im Schwesterwerkzeug \emph{rtheatflow}) und der
...
\begin{beispiel}[Ein Engpass, drei Sichten]
Wie weit die Sichten auseinanderliegen können, zeigt
Referenzszenario~3 (Kapitel~\ref{ch:scenarios}): Sechs E-Autos am
Stranganfang laden mittags mit 22~kW den Überschuss der PV-Anlagen
am Strangende, ein Segment mitten im Strang trägt dabei
$\approx$110\,\% (248~A). Im \ui{Lastfluss} ist genau dieses Kabel
tiefrot --- die Wahrheit. Ein Klick auf \ui{Gemessen} (platziert ist
nur die Stationsmessung): alles unauffällig, denn am Abgang der
Station fließen nur $\approx$102~A (45\,\%) --- mehr sieht auch die
NH-Sicherung dort nie. Noch ein Klick auf \ui{Schätzung}: Ohne
weitere Messgeräte schätzt der Netzbetreiber das überlastete Segment
auf harmlose $\approx$4\,\%; erst mit SMGWs an PV-Anlagen und
Wallboxen trifft die Schätzung die $\approx$110\,\%. Derselbe
Simulationsschritt, drei Welten --- deshalb ist der Sicht-Umschalter
dauerhaft sichtbar.
\end{beispiel}
...
Der Arbeitsbereich-Umschalter \ui{Netz \& Lasten} in der Menüleiste
öffnet die kombinierte Ansicht (auch der Klick auf den Netz-Chip
rechts oben führt hierher). Die linke Spalte gruppiert alle
verfügbaren Netze in
drei aufklappbare Abschnitte --- dieselbe Optik wie die Sektionen
der Live-Seitenleiste, per Klick auf die Kopfzeile auf- und
zugeklappt, die Zahl rechts nennt die Netze im Abschnitt:

\begin{description}
  \item[\ui{Referenznetze (IEEE · CIGRE · Kerber)}] die klassischen
    Referenznetze der Verteilnetz-Literatur, direkt aus pandapower
    übernommen und ohne weitere Daten immer verfügbar: die beiden
    IEEE-Feeder für Mittel- und Niederspannung (\emph{IEEE 33-Bus
    Test Feeder}, 12,66~kV, und \emph{IEEE European LV Test Feeder},
    416~V mit 906 Knoten), die CIGRE-Benchmarknetze für MS und NS
    sowie drei Kerber-Netze (Land, Dorf, Vorstadt --- die deutschen
    NS-Referenztopologien). Impedanzen, Trafos und offene
    Trennstellen sind \emph{exakt} übernommen; nur die Zeitdimension
    ist ergänzt: Jede Last folgt einer Tagesform, deren Abendspitze
    (19:00 Uhr) genau dem Nennwert des publizierten Falls
    entspricht. NS-Anschlüsse gelten als Haushalte, sodass
    Lastgenerator, E-Autos und PV in Schritt~2 auch auf den
    Referenznetzen funktionieren. Ohne Geodaten zeigen diese Netze
    die synthetischen Ansichten (Geographisch/Schaltbild) statt der
    Karte; auch die Struktur-Vorschau in Schritt~3 zeichnet dann ein
    \emph{Prinzipschaltbild}: die Stränge als aufgeräumter Baum von
    der Einspeisung nach rechts, Transformatoren als klassisches
    Zwei-Ringe-Symbol, offene Trennstellen gestrichelt.
...
\begin{beispiel}[Der IEEE-33-Bus-Feeder als Kontrollpunkt]
Der IEEE-33-Bus-Feeder ist \emph{der} Standardfall der
Verteilnetz-Literatur --- unzählige Veröffentlichungen nennen für
seine Spitzenlast (3,715~MW) eine minimale Knotenspannung von
0{,}913~pu. Genau das lässt sich hier nachstellen: Netz wählen,
starten, mit dem Zeitregler auf 19:00 Uhr springen (die Tagesform
erreicht dort die publizierten Nennlasten) --- die Übersicht zeigt
min~U $=$ 0{,}913~pu. Die Testsuite pinnt diese Übereinstimmung
maschinell: Am Spitzenschritt reproduziert der Simulator die
pandapower-Lösung des Original-Referenzfalls auf unter
$2\cdot10^{-6}$~pu, für alle mitgelieferten Referenznetze.
\end{beispiel}
...
\begin{beispiel}[Zeitraffer zum Suchen, Echtzeit zum Besprechen]
Eine typische Vorführung kombiniert die Regler: Zuerst die
\ui{Schrittdauer} auf 0{,}1~s stellen --- der komplette Tag vergeht
in 2{,}4~Minuten, und im Zeitraffer zeichnet sich auf der Karte ab,
\emph{wann} es kritisch wird. Sobald sich der Hauptstrang abends
orange färbt, \ui{Pause} drücken, mit dem \ui{Zeitpunkt}-Regler ein
paar Minuten vor die Spitze zurückspringen und die Schrittdauer auf
1--2~s stellen. Jetzt baut sich die Spitze in mitlesbarem Tempo auf
--- pro Schritt eine Simulationsminute, die Kennzahlen der Übersicht
bleiben lesbar. Bei 1~s Takt dauert der ganze Tag 24~Minuten; der
Zeitraffer ist also das Suchwerkzeug, die Echtzeit der
Besprechungsmodus --- auf die Abendspitze warten muss niemand.
\end{beispiel}
...
Auf der \emph{Karte} öffnet ein Klick auf einen Knoten, eine Leitung
oder einen Transformator ein kleines \textbf{Live-Werte-Popup} direkt
am Element --- es tickt mit jedem Simulationsschritt weiter und
schließt per \texttt{×}, \texttt{ESC} oder Klick auf die freie Karte:

\begin{itemize}[nosep]
  \item \emph{Knoten}: Spannung in Volt und Prozent, Wirk- und
    Blindleistung; am Netzanschluss zusätzlich der Austausch mit der
    überlagerten Ebene.
  \item \emph{Leitung}: Auslastung, Strom, transportierte Leistung
    und Verluste; offene Trennstellen sind gekennzeichnet.
  \item \emph{Transformator}: Auslastung, OS-seitige Leistung und
    Strom.
\end{itemize}

Das Popup respektiert die gewählte \ui{Sicht}: In \ui{Gemessen}
zeigt es an Elementen ohne Messgerät ehrlich \glqq keine Messung ---
unbekannt\grqq{} (ein gemessener Knoten zeigt seine Zählerwerte mit
Zähler-Kennung, ein TAF-7-Zähler erst nach dem ersten vollen
15-min-Fenster), in der \ui{Schätzung} die geschätzten Größen.
Dieselbe Bedienung wie im Schwesterwerkzeug \emph{rtheatflow}.

Das \textbf{Aktionsmenü} öffnet per \emph{Rechtsklick} (in den
synthetischen Ansichten Geographisch/Schaltbild auch weiterhin per
Klick):
...
\begin{beispiel}[Warum der erste Tagesverlauf einen Moment braucht]
Beim ersten angehefteten Diagramm rechnet netzsim den kompletten Tag
vor: 1440 vollständige Lastflüsse. Auf dem 62-Knoten-Netz dieses
Kapitels dauert der erste Klick dadurch rund 10~s --- die
Wiederverwendung der Netzmatrizen drückt die Kosten dabei schon auf
$\approx$5{,}6~ms statt $\approx$11~ms je Schritt. Danach liegen die
Tageskurven \emph{aller} Elemente im Zwischenspeicher: Weitere
Diagramme desselben Tages erscheinen sofort. Nur die Sicht
\ui{Schätzung} kostet einmalig extra, denn dort läuft zusätzlich der
WLS-Sweep der Zustandsschätzung --- je nach Messraster etwa
15--25~s. Wer eine Vorführung vorbereitet, heftet die benötigten
Tagesverläufe deshalb am besten vor Beginn einmal an: Die Wartezeit
ist Physik, kein Hänger.
\end{beispiel}
...
\begin{beispiel}[Zwei Überlasten --- nur eine ist messbar]
Referenzszenario~2: 12 von 24 Haushalten laden zwischen 17:00 und
18:30 Uhr gestaffelt 11-kW-E-Autos. Ab etwa 18:45 Uhr führt der
Strangkopf $\approx$108\,\% bzw.\ 243~A --- genau den Summenstrom,
den auch die NH-Sicherung dort sieht. Hier genügt schon die
Trafo-Messung der Station, um die Überlast zu finden.
Referenzszenario~3 dreht den Fall um: Acht 27-kWp-PV-Anlagen am
Strangende speisen mittags sechs 22-kW-Ladungen nahe der Station
--- die Leistung fließt \emph{innerhalb} des Strangs. Das mittlere
Segment L12 trägt $\approx$110\,\% (248~A), am Stations-Abgang
kommen aber nur 102~A an (45\,\%). Sicherung und Stationsmessung
melden Normalbetrieb, während das Kabel in der Strangmitte
überlastet ist. Die Lehre: Eine Summenmessung erfasst nur, was
durch sie hindurchfließt --- Ausgleichsflüsse zwischen Erzeugern
und Verbrauchern im selben Strang bleiben ohne Messung vor Ort
unsichtbar.
\end{beispiel}
...
\begin{beispiel}[Was keine Standardannahme wissen kann]
Referenzszenario~1: Am Ende eines 500-m-Strangs im ländlichen Netz
speist eine 75-kWp-PV-Anlage. Mittags liegt die Spannung dort in
Wahrheit bei etwa 252~V --- knapp unter der EN-50160-Grenze von
253~V. Die Pseudo-Messungen kennen aber nur
Standard-\emph{Last}annahmen; von der Anlage wissen sie nichts.
Ohne Messung vor Ort schätzt der Netzbetreiber deshalb nur etwa
245~V und hält den Strang für unauffällig --- die drohende
Grenzverletzung existiert in seiner Sicht schlicht nicht. Ein
einziges SMGW an der Anlage ändert alles: Die Schätzung trifft die
Wahrheit auf unter 1~V. Die Lehre: Pseudo-Messungen tragen die
Schätzung dort, wo sich viele Kunden statistisch mitteln; wo eine
einzelne große Anlage das Bild dominiert, ersetzt nichts die echte
Messung an genau diesem Punkt.
\end{beispiel}
...
\begin{beispiel}[Was eine Schätzung kostet]
Die WLS-Schätzung ist der teuerste Rechenschritt der Plattform. Auf
dem 62-Knoten-Vorstadtnetz kostet ein Durchlauf bei Vollmessung
etwa 0,4~s --- das passt bequem in den Standard-Tick von 1~s je
Simulationsschritt, die Schätzung kann also jedem Schritt folgen,
den das Messraster erlaubt. Wechselt man auf den 475-Bus-Bezirk,
steigt der Aufwand auf etwa 1--1,6~s je Durchlauf --- länger als
ein ganzer Tick. Hier greift die automatische Drosselung: Zwischen
zwei Schätzungen zeigt die Ansicht den jeweils letzten Stand, die
Simulation selbst tickt ungebremst weiter. Wer die Schätz-Ansicht
auf dem Bezirk beobachtet, sieht die Werte deshalb in größeren
Sprüngen nachziehen --- das ist kein Fehler, sondern sichtbare
Rechenzeit. Die Lehre: Beobachtbarkeit hat neben den Gerätekosten
...
% ===================================================================
\chapter{Externe Quellen: Live-Daten an einzelnen Knoten}
\label{ch:extern}

Neben synthetischen Tagesprofilen und aufgezeichneten PV-Messtagen
(Kapitel~\ref{ch:realpv}) kennt netzsim eine dritte Datenquelle: den
\emph{externen Knoten}. Seine Einspeisung bzw.\ Last wird nicht aus
einem Profil gelesen, sondern \emph{live von außen} gesetzt --- ein
vollständig ferngesteuerter Knoten mitten im simulierten Netz.
Typische Anwendungen:

\begin{itemize}[nosep]
  \item \textbf{Hardware-in-the-Loop:} ein reales Gerät (Laboraufbau,
    Wechselrichter, Smart-Meter-Auslesung) speist seine Messwerte in
    das simulierte Netz ein.
  \item \textbf{Co-Simulation:} ein externes Skript oder
    Energiemanagementsystem spielt einen Haushalt und reagiert auf
    das, was es beobachtet.
  \item \textbf{Echte Live-Daten:} etwa eine reale Dach-PV-Anlage als
    Live-Feed statt aufgezeichneter Tageskurven.
  \item \textbf{Interaktive Lehre:} eine zweite Person steuert von
    ihrem Laptop aus ,,ihr`` Haus, während die Gruppe zusieht, wie
    das Netz reagiert.
\end{itemize}

\section{Anbinden und beobachten}

Ein Rechtsklick auf einen Knoten öffnet das Element-Menü;
\ui{Externe Quelle anbinden} legt den externen Knoten dort an. Auf
Karte und Schema markiert ihn ein Antennen-Symbol, und seine
Seitenleisten-Sektion bleibt --- wie bei einer Batterie --- dauerhaft
bestehen. Sie zeigt (Abbildung~\ref{fig:ui-extern}):

\begin{itemize}[nosep]
  \item den \textbf{angewandten Wert} in kW/kvar --- vorzeichenbehaftet:
    positiv = Last, negativ = Einspeisung;
  \item das \textbf{Alter des letzten Telegramms} in Sekunden;
  \item die \textbf{Wertgrenze} ($\pm$50\,kW ab Werk) und die
    Timeout-Richtlinie;
  \item den Graphen \ui{empfangen}: die tatsächlich angekommenen Werte
    des laufenden Tages. Ein externer Knoten hat keine Zukunft --- wo
    andere Knoten ihre Profil-Tageskurve zeigen, zeigt er nur, was
    bereits eintraf.
\end{itemize}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{ui-extern}
  \caption{Externe Quelle am Strangende: Der Knoten empfängt live
    $-6{,}4$\,kW Einspeisung (Antennen-Symbol am Knoten, Sektion
    rechts). Der \emph{empfangen}-Graph ist der Tagesring der
    tatsächlich angekommenen Werte --- eine Prognose gibt es für
    einen ferngesteuerten Knoten nicht.}
  \label{fig:ui-extern}
\end{figure}

\section{Briefkasten mit Sample-and-Hold}

Die Engine wartet nie auf das Netzwerk: Externe Clients
\emph{schieben} Sollwerte, wann immer sie wollen; die Simulation liest
pro Rechenschritt den \emph{jüngsten} Wert (Briefkasten-Prinzip,
,,latest wins``). Bleiben die Telegramme aus, wird der Knoten nach
\texttt{hold\_s} Sekunden (Vorgabe 30) als \textbf{stumm} markiert ---
die Sektion zeigt dann eine Warnung mitsamt der Konsequenz der
gewählten Richtlinie:

\begin{description}
  \item[\texttt{hold}] der letzte Wert wird eingefroren
    weiterverwendet (Vorgabe);
  \item[\texttt{zero}] der Knoten fällt auf 0\,kW zurück.
\end{description}

Ein stummer Feed ist damit \emph{sichtbar}, verfälscht die Simulation
aber nie unbemerkt. Die Wertgrenze \texttt{p\_max\_kw} schützt in
beide Richtungen: Werte darüber weist die API ab, und die
Zustandsschätzung (Kapitel~\ref{ch:observability}) verwendet dieselbe
Grenze als Unsicherheit des Knotens --- ein unbeobachteter externer
Knoten ist für den Netzbetreiber bewusst der am schwersten schätzbare.
Überlast-Regler und Netzampel fassen externe Knoten grundsätzlich
nicht an: ,,ferngesteuert`` heißt, die Steuerbox hat dort keinen
Hebel.

\section{Werte einspeisen}

Der heiße Pfad ist ein einzelner REST-Aufruf --- jede Sprache, jedes
Tool mit HTTP genügt:

\begin{lstlisting}
curl -X PUT http://localhost:8000/ext/1/value \
     -H "Content-Type: application/json" \
     -d '{"p_kw": -6.4}'
\end{lstlisting}

Als Referenz-Client liegt \texttt{scripts/ext\_feed.py} bei. Er legt
den Knoten bei Bedarf selbst an und schiebt einmal pro Sekunde einen
Wert:

\begin{lstlisting}
# synthetische PV-Halbwelle, kein Zubehoer noetig:
python scripts/ext_feed.py --bus 3

# reale Dach-PV aus einer InfluxDB (Raspberry Pi),
# 600-W-Anlage auf ein 15-kW-Dach skaliert:
python scripts/ext_feed.py --bus 3 --source pi --scale 25
\end{lstlisting}

\begin{beispiel}[Was von der realen Anlage im Netz ankommt]
Die reale Anlage hinter \texttt{--source pi} ist klein: In dem
Moment, in dem der Feed startet, liefert sie gerade 39~W. Mit
\texttt{--scale 25} tritt sie im simulierten Netz als
15-kW-Dachanlage auf: Das Skript multipliziert jeden Messwert und
schiebt ihn vorzeichenbehaftet als Einspeisung --- die Sektion des
Knotens zeigt als angewandten Wert $-0{,}975$\,kW, und das
Telegrammalter springt mit jedem Sollwert auf null zurück. Die
Rechnung lohnt das Nachvollziehen: 39~W $\times$ 25 = 975~W, mit
Einspeise-Vorzeichen also $-0{,}975$\,kW. Zieht über der realen
Anlage eine Wolke durch, sinkt die Einspeisung im simulierten Netz
mit --- echte Wetterdynamik an einem einzelnen Knoten, während der
Rest des Netzes weiter seinen Profilen folgt.
\end{beispiel}

\begin{beispiel}[Der Stumm-Fall gehört zur Vorführung]
Das Skript mit \texttt{Strg+C} zu stoppen ist Teil der Demo: Nach
\texttt{hold\_s} Sekunden erscheint die Warnung ,,Quelle stumm``, und
je nach Richtlinie hält der Knoten seinen letzten Wert oder fällt
hörbar auf 0\,kW --- auf der Karte springt die Leitungsfärbung. So
wird diskutierbar, was ein Netzbetreiber mit ausgefallenen
Live-Feeds tun sollte.
\end{beispiel}

Externe Knoten sind Teil der Szenario-Rezepte
(Kapitel~\ref{ch:scenarios}): Gespeichert wird die \emph{Platzierung}
(Knoten, Name, Timeout-Richtlinie, Wertgrenze), nie der letzte
Live-Wert --- ein geladenes Szenario startet mit stummen, frischen
Knoten. Der Daten-Export (Kapitel~\ref{ch:export}) spielt Tage
deterministisch nach; externe Knoten liefern dabei 0\,kW, denn ein
Export darf nicht davon abhängen, was ein Live-Feed zufällig gerade
sendete.
...
\begin{beispiel}[Was die Bezirks-Schätzung kostet]
Eine WLS-Schätzung ist echte Rechenarbeit, und sie wächst mit dem
Netz. Auf dem 62-Knoten-Vorstadtnetz kostet ein Durchlauf bei
Vollmessung etwa 0{,}4~s --- beim Standard-Takt von 1~s pro Schritt
kann die Sicht \ui{Schätzung} dort also fast jedem Schritt folgen.
Auf dem 475-Knoten-Bezirk braucht derselbe Durchlauf 1 bis 1{,}6~s,
also länger als ein Simulationsschritt. Die Folge ist am Bildschirm
sichtbar: Die Bezirks-Schätzung erneuert sich nur alle paar
Schritte, dazwischen bleibt der zuletzt berechnete Stand stehen.
Das ist kein Fehler, sondern Absicht --- die Engine wartet nie auf
die Schätzung. Auch ein realer Netzbetreiber rechnet seinen
Netzzustand nicht sekündlich neu; die Leitwarte arbeitet mit dem
jeweils jüngsten verfügbaren Schätzergebnis.
\end{beispiel}
...
\begin{beispiel}[Zwei Wege, zwei Größenordnungen]
Zum Gefühl für Dauer und Datenmengen zwei verifizierte Eckwerte vom
Vorstadtnetz. Ein Tages-Export über zwei Tage rechnet 2880 Schritte
in 62~Sekunden durch, während die laufende Simulation ungestört
weitertickt; das fertige Paket ist ein ZIP von rund 5~MiB. Eine
kurze Live-Aufzeichnung von 91 Schritten bleibt dagegen bei
241~KiB --- pro Schritt schreibt sie in \texttt{buses.csv} 62
Zeilen, eine je Knoten, dazu die Zeilen der Leitungen, Messwerte
und Schätzblöcke. Die Paketgröße wächst also schlicht mit
Schrittzahl und Netzgröße. Für ein Übungsblatt heißt das: lieber
gezielt die zwei relevanten Tage exportieren als lange live
aufzeichnen --- beide Pakete sind identisch aufgebaut, der Export
ist nur schneller fertig.
\end{beispiel}
...
\texttt{http://localhost:8000}). Die \emph{vollständige} Referenz
aller Endpunkte steht in \texttt{docs/API.md} (aus dem Code
generiert, siehe Abschnitt~\ref{sec:weiterdoku}) und interaktiv in
der eingebauten Swagger-Oberfläche unter \api{/docs} --- dort lässt
sich jeder Aufruf direkt im Browser ausprobieren. Auswahl der
...
\begin{description}
  \item[Die Oberfläche zeigt \glqq kein Netz\grqq{} / Fehler beim Laden]
    Läuft das Backend? \api{GET /health} muss antworten, und zwar mit
    \texttt{"app": "netzsim"} in der Antwort --- genau daran erkennt
    man, dass wirklich der Netzsimulator auf dem Port sitzt und nicht
    eine andere Anwendung mit denselben Standard-Ports. Achtung: Der
    Starter weicht automatisch auf den nächsten freien Port aus
    (8001, 8002, …), wenn Port 8000 fremdbelegt ist --- der wirklich
    verwendete Port steht im Titel des Backend-Fensters und in der
    Abschlusszeile des Starters. Im Entwicklungsmodus zusätzlich
    prüfen, dass das Backend unter \texttt{127.0.0.1} antwortet
    (nicht nur unter \texttt{localhost}).
  ...
  \item[Port 8000 ist belegt] Der Starter löst das inzwischen
    selbst: Ein bereits laufender Netzsimulator wird erkannt und
    weiterverwendet, eine fremde Anwendung auf dem Port führt zum
    automatischen Ausweichen auf den nächsten freien Port. Manuell
    festlegen lässt sich der Port weiterhin über
    \texttt{NETZSIM\_PORT}; die Prozess-ID zu einem belegten Port
